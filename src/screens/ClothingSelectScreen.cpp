...
class ClothingSelectScreen final : public Screen {
public:
  void enter() override {
    count_ = 1;
    selected_ = 0;
    ...
    for (int i = 0; i < ITEM_COUNT && count_ < MAX_ITEMS + 1; ++i) {
      if (ITEMS[i].category == ItemCategory::Clothing && inventory.owned(ITEMS[i].id)) {
        choices_[count_++] = ITEMS[i].id;
      }
    }
    selectEquipped(inventory.equippedClothing);
  }
  ...
  void onButtonB() override {
    App::profile().inventory.equippedClothing = choices_[selected_];
    celebrateMs_ = 950;
  ...
  const char* name() const override { return "CLOTHING SELECT"; }
...
private:
  ...
  void drawNonePreview(IconId icon) const {
    auto& canvas = Gfx::c();
    UI::drawIcon(icon, 160, 101, 65, UI::theme().accent);
  ...
  void drawInfo(uint16_t itemId, bool equipped) const {
    ...
    canvas.drawString(itemId == ITEM_NONE ? "NONE" : (item ? item->name : "CLOTHES"), 160, 177);
    if (equipped) {
...
ClothingSelectScreen instance;
ScreenRegistrar registrar(ScreenId::ClothingSelect, instance);
